...
/* not working - changeDirection(f,f) change only ballMovementVector, not angle
 *TEST_F(BallClassTest, changeDirection) {
 *	EXPECT_EQ(bAlfa, ball->getBallAlfa());
 *	ball->changeDirection(-1,-1);
 *	EXPECT_EQ(bAlfa+180, ball->getBallAlfa());}
 */
TEST_F(BallClassTest, changeDirection) {
	EXPECT_EQ(bAlfa, ball->getBallAlfa());
	ball->changeDirection(20.);
	EXPECT_EQ(bAlfa + 20., ball->getBallAlfa());
}